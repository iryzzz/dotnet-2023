...
public class RequestController : ControllerBase
{
    ...
    public async Task<ActionResult<BookGetDto>> Get()
    {
        ...
        var bookSelect = await (from book in _context.Books
                                ...
                                select new { Book = book, Card = card })
                        .ToListAsync();

        var groupedBooks = bookSelect.GroupBy(x => x.Book.Id)
                                     .Select(g => new
                                     {
                                         Book = _mapper.Map<Book, BookGetDto>(g.First().Book),
                                         Cards = g.Select(x => x.Card).ToList()
                                     })
                                     .ToList();
        var issuedBooks = groupedBooks.Select(group => group.Book).OrderBy(x => x.Name).ToList();
    ...
    public async Task<ActionResult<ReaderGetDto>> GetDelayReaders()
    {
        ...
        var maxDelay = await (from card in _context.Cards
                              ...
                              select new
                              {
                                  ...
                                  MaxDelay = card.DateReturn.Subtract(card.DateIssue).TotalDays - card.DayCount
                              }).ToListAsync();
